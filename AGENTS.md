...
```
2.0.1 ✅  →  2.1.0 ✅  →  2.2  →  2.3  →  3.0
hotfix      正确性债务    API 拉齐   测试/CI   综合前进
patch       minor         minor      patch     major
```
...
| **2.3** | ⏳ 待开始 | 测试/CI 大补：WASM 测试、cross-format 矩阵、CI 加 WASM/MSRV/clippy 矩阵、CODE_WIKI 重写 |
| **3.0** | ⏳ 待开始 | 综合前进：IMSC/CEA-708/SMPTE-TT + AI 集成 + zero-copy + npm/PyO3/C 绑定 |

**当前版本**：`2.1.0`（见 `Cargo.toml`）。**下一版本**：`2.2`（API 拉齐）。
