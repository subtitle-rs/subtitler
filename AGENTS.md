...
```
2.0.1 ✅  →  2.1.0 ✅  →  2.2.0 ✅  →  2.3.0 ✅  →  2.4.0 ✅  →  2.4.1 ✅  →  3.0
hotfix      正确性       API 拉齐    测试/CI      gap收编    外部测试修复   综合前进
patch       patch        minor       patch        minor       patch          major
```
...
| 版本 | 状态 | 范围 |
|------|------|------|
| **2.0.1** | ✅ 已发布（GitHub Release + tag；crates.io 待手动） | hotfix：clippy/CHANGELOG/README/Cargo.toml 文档对齐 |
| **2.1.0** | ✅ 已发布（GitHub Release + tag；crates.io 待手动） | 6 项 P1 正确性修复 + dependabot 升级。13 新测试，总 286 |
| **2.2.0** | ✅ 已发布（crates.io ✅） | API 拉齐：13 格式 generate/parse_stream/write_stream 统一 + TTML write_stream_async + header 保留。7 新测试，总 293 |
| **2.3.0** | ✅ 已发布（crates.io ✅） | 测试/CI 夯实：WASM 测试、cross-format 矩阵 23 对、proptest 扩展、chardetng fixture、CI WASM/MSRV/clippy-matrix/bench jobs。32 新测试，总 325 |
| **2.4.0** | ✅ 已发布（crates.io ✅） | gap analysis P1 收编：DFXP + Whisper JSON + 去重 PipelineOp + normalize 4 扩展。15 新测试，总 340 |
| **2.4.1** | ✅ 已发布（crates.io ✅） | 外部测试修复：SCC 文本解码 P1、DFXP namespace、SubViewer 检测、SBV 两行格式、iTT SMPTE。4 新测试，总 344 |
...
> **2.4 + 3.0 + 3.1 是原 3.0 拆分而来**（2026-07-18，基于 `docs/subtitler-vs-editingtools-gap-analysis.md`）。拆分理由见 spec §1。

**当前版本**：`2.4.1`（见 `Cargo.toml`）。**下一版本**：`3.0`（综合前进）。
